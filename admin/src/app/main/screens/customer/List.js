...
function Customer(props) {
	...
	useEffect(() => {
		if (searchText.length !== 0) {
			const res = axios.get(`http://localhost:5000/api/users`).then(resp =>
				setData(
					resp.data.user.filter(item => {
						let { firstName, lastName, email, phone } = item;
			...
			const res = axios.get(`http://localhost:5000/api/users`).then(resp => setData(resp.data.user));
		}
	...
	const columns = [
		...
		{ id: 'email', label: 'TITLE', width: 250 },
		{ id: 'phone', label: 'Phone', width: 120 },
		{ id: 'isAdmin', label: 'Admin', width: 120 }
	];
